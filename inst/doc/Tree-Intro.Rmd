...
In this vignette, we saw how to build a tree and then customize the tree by altering the tree size, the spline basis, and the projection grid. We saw that a spline tree can be printed, plotted, and summarized, and we showed some examples of working with spline tree components by hand. We then saw how to use a tree to predict response values or coefficients, and finally we saw $R^2$ evaluation metrics for trees. Along the way, we also saw how to quickly determine the size of a tree or quickly determining how similar two trees are. 

One aspect of our trees that we have yet to address is variable importance. The different trees that we built along the way sometimes included different sets of split variables. Faced with this inconsistency, how do we know which of the split variables contribute most to the portion of varation in BMI trajectory that we are able to explain? To answer this question most accurately, we must use spline forests. For more information, see the *Forest Building with SplineTree* Vignette. 
