...
predict_y_RF <- function(forest, method = "oob", testdata=NULL) {
  ...
  tvar <- forest$tvar
  idvar <- forest$idvar
  coeffPreds <- predict_coeffs_RF(forest, method, testdata)
  preds <- rep(NA, NROW(forest$data))
  
  for (i in 1:NROW(forest$data)) {
    ID <- forest$data[i, ][[idvar]]
    idIndex <- which(Xdata[[idvar]] == ID)
    coeffs <- coeffPreds[[idIndex]]
    
    ### Assumes that forest includes intercept. 
    basisMat <- cbind(1, bs(forest$data[i, ][[tvar]], 
