...
When we lower the ``cp`` to 0.001, the tree has 28 terminal nodes. This tree will be difficult to plot and interpret. Let's try a value in-between and set ``cp`` to be 0.005. 
...
```{r, eval=FALSE}
fourth_tree <- splineTree(split_formula, tformula, idvar, data, minNodeSize = 20, cp = 0.005, nGrid = 30)
